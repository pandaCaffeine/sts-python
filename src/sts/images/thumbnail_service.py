...
from sts.images.image_processor import resize_image_async
...
class ThumbnailService:
    ...
    async def get_thumbnail(self, bucket: str, file_name: str, etag: str | None) -> Response:
        scan_result = await self._file_storage_scanner.scan_file(bucket, file_name)
        result: Response
        ...
        if scan_result.status == ScanStatus.BUCKET_NOT_FOUND:
            ...
            result = await self._get_file_response(scan_result.source_file_stat, etag)
            ...
            result = await self._get_file_response(scan_result.file_stat, etag)
        else:
            assert scan_result.source_file_stat, "scan_result doesn't have source_file_stat"
            assert scan_result.bucket_settings, "scan_result doesn't have bucket_settings"
            result = await self._create_thumbnail_and_upload(scan_result.source_file_stat,
                                                       scan_result.bucket_settings,
                                                       bucket)

        return result

    async def get_thumbnail_by_alias(self, source_bucket: str, file_name: str, alias: str, etag: str | None) -> Response:
        result: Response
        bucket = self._file_storage_scanner.find_bucket_by_alias(source_bucket, alias)
        if bucket:
            result = await self.get_thumbnail(bucket, file_name, etag)
        else:
    ...
    async def _create_thumbnail_and_upload(self, source_file_stat: StorageFileItem,
                                     bucket_settings: BucketSettings, bucket: str) -> Response:
        image_data = await self._storage_client.load_file(source_file_stat.bucket, source_file_stat.file_name)
        if not image_data:
            self._logger.debug("Source file was not found")
            return NOT_FOUND_RESPONSE

        self._logger.debug("Source file was loaded into memory")
        thumbnail = await resize_image_async(image_data,
        ...
        put_result = await self._storage_client.put_file(bucket, source_file_stat.file_name, thumbnail.data,
    ...
    async def _get_file_response(self, file_storage_item: StorageFileItem, etag: str | None) -> Response:
        ...
        object_stream = await self._storage_client.open_stream(file_storage_item.bucket, file_storage_item.file_name)
